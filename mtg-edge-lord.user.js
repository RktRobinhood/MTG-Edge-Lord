...
// ==/UserScript==
(()=>{var Se="edge-v1",ve=Object.freeze({bracketFit:.4,archetypeDepth:.4,retention:.2}),Ce=Object.freeze([0,0,1,1,.6]),S=30,g=Object.freeze({zeroAt:500,peakFrom:1e3,peakTo:3e3}),$=3e3,Ee=Object.freeze([{tier:"meta",maxRank:500},{tier:"rare",maxRank:1e3},{tier:"edge",maxRank:3e3},{tier:"uncharted",maxRank:1/0}]),Re=e=>Math.round(e*10)/10,v=e=>Math.round(e*1e3)/1e3,G=e=>Math.max(0,Math.min(1,Number(e)||0));function K(e){return!Number.isFinite(e)||e<=0?"uncharted":Ee.find(t=>e<=t.maxRank).tier}function W(e){return!Number.isFinite(e)||e<=g.zeroAt||e>g.peakTo?0:e>=g.peakFrom?1:v((e-g.zeroAt)/(g.peakFrom-g.zeroAt))}function Te(e){if(!Array.isArray(e)||e.length!==5)return null;let t=e.map(o=>Math.max(0,Number(o)||0)),r=t.reduce((o,s)=>o+s,0);if(r<S)return null;let n=t.reduce((o,s,l)=>o+s*Ce[l],0);return v(n/r)}function Ae(e){if(!Array.isArray(e)||e.length<4)return null;let t=e.map(s=>Math.max(0,Number(s)||0)),r=Math.floor(t.length/2),n=U(t.slice(0,r)),o=U(t.slice(r));return n<=0?o>0?1:null:v(G(o/n/2))}function Y(e,t=ve){let r=e.popularity?.edhrecRank,n=K(r);if(!Number.isFinite(r)||r<=0)return{unscored:!0,reason:"No EDHREC rank yet, so obscurity cannot be judged.",tier:n};if(r>$)return{unscored:!0,reason:`Past EDHREC rank ${$}, where the evidence to say this works does not exist.`,tier:n};let o={bracketFit:Te(e.bracketCounts),archetypeDepth:e.archetypeDepth===void 0?null:G(e.archetypeDepth),retention:Ae(e.retentionTrend)},s=Object.entries(o).filter(([,k])=>k!==null);if(!s.length)return{unscored:!0,reason:w(e)>0?`Only ${w(e)} bracket-tagged decks, below the floor of ${S}.`:"No EDHREC page data has been collected for this commander yet.",tier:n,...e.bracketCounts?{bracketCounts:e.bracketCounts}:{}};let l=s.reduce((k,[A])=>k+t[A],0),p=v(s.reduce((k,[A,we])=>k+we*t[A],0)/l),x=W(r);return{tier:n,obscurity:x,worksScore:p,edgeScore:Re(x*p*100),quality:Object.fromEntries(s),partial:s.length<Object.keys(o).length,reasons:M(e,{obscurity:x,worksScore:p,tier:n,components:o}),modelVersion:Se}}function M(e,{obscurity:t,worksScore:r,tier:n,components:o}={}){return t??=e.obscurity??W(e.popularity?.edhrecRank),r??=e.worksScore??0,n??=e.tier??K(e.popularity?.edhrecRank),o??={bracketFit:e.quality?.bracketFit??null,archetypeDepth:e.quality?.archetypeDepth??null,retention:e.quality?.retention??null},Me(e,{obscurity:t,worksScore:r,tier:n,components:o})}function J(e){let t=e.popularity?.edhrecRank;if(!Number.isFinite(t)||t<=0)return"No EDHREC rank yet, so obscurity cannot be judged.";if(t>$)return`Past EDHREC rank ${$.toLocaleString()}, where the evidence to say this works does not exist.`;let r=w(e);return r>0?`Only ${r} bracket-tagged deck${r===1?"":"s"}, below the floor of ${S}.`:"No EDHREC page data has been collected for this commander yet."}function Me(e,{obscurity:t,worksScore:r,tier:n,components:o}){let s=[];if(t>=1?s.push(`Sits at EDHREC rank ${e.popularity.edhrecRank}, squarely in the Edge tier.`):t>0&&s.push(`Rank ${e.popularity.edhrecRank} is on the edge of the Rare tier, so obscurity counts for less.`),o.bracketFit!==null){let l=Math.round(o.bracketFit*100),p=w(e);s.push(`${l}% of its ${p.toLocaleString()} bracket-tagged decks are built at Bracket 3 or above.`)}else e.bracketCounts&&s.push(`Bracket data shown but not scored: only ${w(e)} tagged decks, below the floor of ${S}.`);return o.archetypeDepth!==null&&s.push(o.archetypeDepth>=.35?"A deep pool of high-synergy cards means there is an archetype here, not just goodstuff.":"A shallow high-synergy pool suggests the deck leans on colour-identity staples."),o.retention!==null&&s.push(o.retention>=.5?"Deck saves are holding or climbing rather than fading after release.":"Deck saves are falling away from their earlier level."),r<.3&&s.push("The evidence that this works is weak, so the Edge score stays low however obscure it is."),n==="rare"&&r>=.5&&s.push("Known enough to have a track record, obscure enough to be worth a second look."),s.slice(0,5)}function w(e){return Array.isArray(e.bracketCounts)?e.bracketCounts.reduce((t,r)=>t+(Number(r)||0),0):0}function U(e){return e.length?e.reduce((t,r)=>t+r,0)/e.length:0}var ht=Object.freeze({cohortPosition:.6,interestToTraction:.4});function Z(e){let t=e?.cohort;if(!t)return[];let r=e.popularity?.deckCount??0,n=[`${Oe(t.position)} of ${t.size} new legends in ${String(t.setCode).toUpperCase()}, with ${r.toLocaleString()} deck${r===1?"":"s"} so far.`],o=e.mentionCount;return n.push(Number.isFinite(o)&&o>0?`${o} deck-tech mention${o===1?"":"s"} against ${r.toLocaleString()} build${r===1?"":"s"}.`:"No deck-tech coverage found yet, so this is cohort position alone."),n}function Oe(e){let t=e%100;return t>=11&&t<=13?`${e}th`:`${e}${["th","st","nd","rd"][e%10]??"th"}`}var O="columnar/1";function Q(e){if(e?.format!==O)throw new Error(`Unsupported columnar format: ${e?.format??"missing"}`);let t=Array.from({length:e.count},()=>({}));for(let[r,n]of Object.entries(e.columns)){let o=Le(n,e.count);for(let s=0;s<e.count;s+=1)o[s]!==null&&Ne(t[s],r,o[s])}return t}function Le(e,t){if(e.kind==="raw")return e.values;if(e.kind==="tokens")return e.index.map(r=>r===null?null:r.map(n=>e.vocabulary[n]));if(e.kind==="dict")return e.index.map(r=>r===-1?null:X(e.keys[r]));if(e.kind==="sparse"){let r=Array.from({length:t},()=>X(e.fill));return e.index.forEach((n,o)=>{r[n]=e.values[o]}),r}throw new Error(`Unsupported column kind: ${e.kind}`)}function X(e){return typeof e=="object"&&e!==null?structuredClone(e):e}function Ne(e,t,r){let n=t.split("."),o=e;for(let s of n.slice(0,-1))De(o[s])||(o[s]={}),o=o[s];o[n.at(-1)]=r}function De(e){return typeof e=="object"&&e!==null&&!Array.isArray(e)}var xt=Object.freeze({trendZscore:3,momentum:1,diamondScore:1,edgeScore:1,cohortScore:1,"quality.bracketFit":3,"quality.archetypeDepth":3,"quality.retention":3,price:2});function ee(e){let t=e?.commanders;return Array.isArray(t)?t:t?.format===O?Q(t):[]}var Ie="mtg-edge-lord",u="datasets";var C=null;function L(){return C||(C=new Promise(e=>{if(!globalThis.indexedDB)return e(null);let t;try{t=indexedDB.open(Ie,1)}catch{return e(null)}t.onupgradeneeded=()=>{t.result.objectStoreNames.contains(u)||t.result.createObjectStore(u)},t.onsuccess=()=>e(t.result),t.onerror=()=>e(null),t.onblocked=()=>e(null)}),C)}async function N(e){let t=await L();return t?new Promise(r=>{try{let n=t.transaction(u,"readonly").objectStore(u).get(e);n.onsuccess=()=>r(n.result??null),n.onerror=()=>r(null)}catch{r(null)}}):null}async function D(e,t){let r=await L();return r?new Promise(n=>{try{let o=r.transaction(u,"readwrite");o.objectStore(u).put(t,e),o.oncomplete=()=>n(!0),o.onerror=()=>n(!1),o.onabort=()=>n(!1)}catch{n(!1)}}):!1}async function te(e){let t=await L();if(t)try{t.transaction(u,"readwrite").objectStore(u).delete(e)}catch{}}var oe="https://rktrobinhood.github.io/MTG-Edge-Lord/data",ae="https://raw.githubusercontent.com/RktRobinhood/MTG-Edge-Lord/main/data",re="datasets:v2",I="commander-detail:v2",je="mtg-edge-lord:data:v1";async function se({force:e=!1}={}){Fe();let t=await N(re);try{let r;try{r=await ne(oe,t,e)}catch{r=await ne(ae,t,e)}if(r.cached)return{...t,stale:!1};let n={manifest:r.manifest,...r.datasets,cachedAt:new Date().toISOString()};return await D(re,n),t?.manifest?.dataVersion!==r.manifest.dataVersion&&await te(I),{...n,stale:!1}}catch(r){if(t)return{...t,stale:!0,error:r.message};throw r}}async function ie(e){let t=await N(I);if(t?.dataVersion===e)return t.detail;for(let r of[oe,ae])try{let{detail:n}=await f(`${r}/commander-detail.json`);return await D(I,{dataVersion:e,detail:n}),n}catch{}return t?.detail??{}}async function ne(e,t,r){let n=await f(`${e}/manifest.json?ts=${Date.now()}`);if(!r&&t?.manifest?.dataVersion===n.dataVersion)return{cached:!0};let[o,s,l,p,x]=await Promise.all([f(`${e}/findings.json`),f(`${e}/commanders.json`).then(_e),f(`${e}/hidden-cards.json`),f(`${e}/community-resources.json`),f(`${e}/relationships/card-commander.json`)]);return{manifest:n,datasets:{findings:o,commanders:s,cards:l,resources:p,relationships:x}}}function _e(e){return{schemaVersion:e?.schemaVersion??1,commanders:ee(e)}}function Fe(){try{localStorage.removeItem(je)}catch{}}function f(e){let t=globalThis.GM?.xmlHttpRequest??globalThis.GM_xmlhttpRequest;return t?new Promise((r,n)=>t({method:"GET",url:e,headers:{Accept:"application/json"},onload:o=>{if(o.status<200||o.status>=300)return n(new Error(`HTTP ${o.status}`));try{r(JSON.parse(o.responseText))}catch{n(new Error("Backend returned invalid JSON."))}},onerror:()=>n(new Error("Could not reach the MTG Edge Lord data backend."))})):fetch(e).then(ze).then(r=>r.json())}function ze(e){if(!e.ok)throw new Error(`HTTP ${e.status}`);return e}function ce(e,t=220){let r=String(e??"").trim();if(r.length<=t)return r;let n=r.slice(0,t),o=n.lastIndexOf(" ");return`${(o>t*.6?n.slice(0,o):n).replace(/[\s,;:.]+$/,"")}\u2026`}function qe(e){let t=e?.source?.url;return typeof t=="string"&&/^https:\/\//.test(t)}function le(e){return(e??[]).filter(qe)}function j(e){let t=String(e?.creator??"").trim(),r=String(e?.name??"").trim();return t&&r&&t.toLowerCase()!==r.toLowerCase()?`${t} \xB7 ${r}`:t||r||"Source"}var Be=["W","U","B","R","G"],R=Object.freeze([{id:"edge",label:"Edge score"},{id:"momentum",label:"Momentum"},{id:"worksScore",label:"How well it works"},{id:"price",label:"Cheapest first"},{id:"released",label:"Most recent"},{id:"name",label:"Alphabetical"},{id:"rank",label:"EDHREC rank"},{id:"deckCount",label:"Deck count (popularity)"}]),pe="edge",fe=Object.freeze([{id:"includes",label:"Includes"},{id:"exact",label:"Exactly"},{id:"atMost",label:"At most"}]),F=Object.freeze({query:"",colors:[],colorMode:"includes",theme:"",functionalTag:"",creatureType:"",tier:"",minBracketFit:"",minRank:"",maxRank:"",minManaValue:"",maxManaValue:"",maxPrice:"",releasedAfter:"",scoredOnly:!1,sort:pe});function He(e){return e.map(t=>{let r=Y(t);return r.unscored?{...t,tier:r.tier}:{...t,tier:r.tier,obscurity:r.obscurity,worksScore:r.worksScore,quality:r.quality}})}function he(e){let t=He(e),r=t.map(n=>[n.name,...n.themes??[],...n.functionalTags??[],...n.creatureTypes??[],...n.types??[]].join(" ").toLowerCase());return{commanders:t,haystacks:r,themes:_(t,"themes"),functionalTags:_(t,"functionalTags"),creatureTypes:_(t,"creatureTypes")}}function _(e,t){let r=new Map;for(let n of e)for(let o of n[t]??[])r.set(o,(r.get(o)??0)+1);return[...r.entries()].sort((n,o)=>o[1]-n[1]||n[0].localeCompare(o[0])).map(([n])=>n)}function me(e,t){let r=String(t.query??"").toLowerCase().split(/\s+/).filter(Boolean),n=[];for(let o=0;o<e.commanders.length;o+=1){if(r.length&&!r.every(l=>e.haystacks[o].includes(l)))continue;let s=e.commanders[o];Pe(s,t)&&n.push(s)}return n}function Pe(e,t){let r=e.popularity?.edhrecRank;return!(!Ve(e.colorIdentity,t.colors,t.colorMode)||t.theme&&!(e.themes??[]).includes(t.theme)||t.functionalTag&&!(e.functionalTags??[]).includes(t.functionalTag)||t.creatureType&&!(e.creatureTypes??[]).includes(t.creatureType)||t.tier&&e.tier!==t.tier||t.scoredOnly&&e.edgeScore===void 0||t.minBracketFit!==""&&!(e.quality?.bracketFit>=Number(t.minBracketFit))||!de(r,t.minRank,t.maxRank)||!de(e.manaValue,t.minManaValue,t.maxManaValue)||t.maxPrice!==""&&e.price!==void 0&&e.price>Number(t.maxPrice)||t.releasedAfter&&(e.releasedAt??"")<t.releasedAfter)}function de(e,t,r){return t===""&&r===""?!0:!(!Number.isFinite(e)||t!==""&&e<Number(t)||r!==""&&e>Number(r))}function Ve(e,t,r){if(!t?.length)return!0;if(e===void 0)return!1;let n=Be.filter(o=>t.includes(o));return r==="exact"?e===n.join(""):r==="atMost"?[...e].every(o=>n.includes(o)):n.every(o=>e.includes(o))}function ge(e,t){let r=ue[t]??ue[pe];return[...e].sort(r)}var E=e=>(t,r)=>{let n=e(t),o=e(r);return n===void 0&&o===void 0?b(t,r):n===void 0?1:o===void 0?-1:o-n||b(t,r)},Ue=e=>(t,r)=>{let n=e(t),o=e(r);return n===void 0&&o===void 0?b(t,r):n===void 0?1:o===void 0?-1:n-o||b(t,r)};function b(e,t){return(e.popularity?.edhrecRank??1/0)-(t.popularity?.edhrecRank??1/0)}var ue={edge:E(e=>e.edgeScore),momentum:E(e=>e.momentum),worksScore:E(e=>e.worksScore),price:Ue(e=>e.price),released:(e,t)=>String(t.releasedAt??"").localeCompare(String(e.releasedAt??""))||b(e,t),name:(e,t)=>e.name.localeCompare(t.name),rank:b,deckCount:E(e=>e.popularity?.deckCount)};var be=`
...
`;var m=60,Ge=140,ke="mtg-edge-lord:filters",Ke=[{id:"W",label:"W"},{id:"U",label:"U"},{id:"B",label:"B"},{id:"R",label:"R"},{id:"G",label:"G"}],We=[{id:"",label:"Any tier"},{id:"edge",label:"Edge (1,000\u20133,000)"},{id:"rare",label:"Rare (500\u20131,000)"},{id:"meta",label:"Meta (top 500)"},{id:"uncharted",label:"Uncharted (3,000+)"}],a={tab:"search",filters:{...F,...lt()},shown:m,expanded:null,detail:null,index:null,data:null,loading:!0,error:null},ye=null,P=document.createElement("div");P.id="mtg-edge-lord-root";var d=P.attachShadow({mode:"open"});document.body.append(P);Ye();Je();function Ye(){d.innerHTML=`<style>${be}</style>
    ...
    <section id="panel" aria-label="MTG Edge Lord" hidden>
      ...
      <nav>${B("search","Search")}${B("discover","Recent finds")}${B("card","Card-first")}</nav>
      <main></main>
    </section>`,d.getElementById("toggle").addEventListener("click",()=>{let e=d.getElementById("panel");e.hidden=!e.hidden}),d.querySelector("nav").addEventListener("click",e=>{let t=e.target.closest("button[data-tab]");t&&(a.tab=t.dataset.tab,a.shown=m,V())})}async function Je(){try{a.data=await se(),a.index=he(a.data.commanders.commanders)}catch(e){a.error=e.message}finally{a.loading=!1,V()}}function V(){d.querySelectorAll("nav button").forEach(t=>t.classList.toggle("active",t.dataset.tab===a.tab)),d.getElementById("status").textContent=Ze();let e=d.querySelector("main");if(a.loading)return void(e.innerHTML='<div class="empty">Loading\u2026</div>');if(a.error)return void(e.innerHTML=`<div class="empty">${i(a.error)}</div>`);e.innerHTML=`
    ...
    <div class="row">
      <select data-field="theme" aria-label="Theme">${h([["","Any theme"],...a.index.themes.map(t=>[t,T(t)])],e.theme)}</select>
      <select data-field="functionalTag" aria-label="Mechanic">${h([["","Any mechanic"],...a.index.functionalTags.map(t=>[t,T(t)])],e.functionalTag)}</select>
    ...
    <div class="row">
      ...
      <select data-field="sort" aria-label="Sort">${h(R.map(t=>[t.id,t.label]),e.sort)}</select>
    ...
    ${ot(e)}
    ${at(e)}
    <div class="chips">${(e.themes??[]).slice(0,5).map(o=>`<span class="chip">${i(T(o))}</span>`).join("")}</div>
    ${$e(e.findingIds)}
    ...
    <div class="credit">
      <span class="source">${i(j(e.source))}</span>
    ...
    <span class="readon">Read it at ${i(j(e.source))} \u2197</span>
  </a>`}function ct(e){let t=e.commanders.map(r=>`<a class="chip" href="https://edhrec.com/commanders/${encodeURIComponent(r.slug)}" target="_blank" rel="noopener noreferrer">${i(r.name)} \xB7 ${r.relationshipScore}</a>`).join("");return`<article class="card"><h3>${i(e.name)}</h3><div class="chips">${t}</div>${$e(e.findingIds)}</article>`}function $e(e=[]){let t=new Set(e),r=(a.data.resources.resources??[]).filter(n=>t.has(n.findingId)&&typeof n.url=="string"&&n.url.startsWith("https://")).slice(0,3);return r.length?`<p class="sources">${r.map(n=>`<a href="${c(n.url)}" target="_blank" rel="noopener noreferrer">${i(n.creator)} \xB7 ${ut(n.resourceDepth)} \u2197</a>`).join(" \xB7 ")}</p>`:""}function xe(e,t){let r=String(t??"").trim().toLowerCase();return!r||String(e).toLowerCase().includes(r)}function q(){try{sessionStorage.setItem(ke,JSON.stringify(a.filters))}catch{}}function lt(){try{return JSON.parse(sessionStorage.getItem(ke))??{}}catch{return{}}}function dt(e){return R.find(t=>t.id===e)?.label??R[0].label}function T(e){return String(e).replaceAll("-"," ").replace(/\b[a-z]/g,t=>t.toUpperCase())}function h(e,t){return e.map(([r,n])=>`<option value="${c(r)}" ${String(t)===String(r)?"selected":""}>${i(n)}</option>`).join("")}function B(e,t){return`<button type="button" data-tab="${e}" class="${a.tab===e?"active":""}">${t}</button>`}function ut(e){return i(String(e).replaceAll("_"," "))}function H(e){return`<div class="empty">${i(e)}</div>`}function i(e){let t=document.createElement("span");return t.textContent=String(e),t.innerHTML}function c(e){return i(e).replaceAll('"',"&quot;")}})();

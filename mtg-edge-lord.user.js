...
// ==/UserScript==
(()=>{var qe="edge-v2",Pe=Object.freeze({bracketFit:.4,archetypeDepth:.4,retention:.2}),Ue=Object.freeze([0,0,1,1,.6]),U=30,y=Object.freeze({zeroAt:500,peakFrom:1e3,peakTo:3e3}),w=3e3,Ge=Object.freeze([{tier:"meta",maxRank:500},{tier:"rare",maxRank:1e3},{tier:"edge",maxRank:3e3},{tier:"uncharted",maxRank:1/0}]),Ve=e=>Math.round(e*10)/10,T=e=>Math.round(e*1e3)/1e3,ue=e=>Math.max(0,Math.min(1,Number(e)||0));function S(e){return!Number.isFinite(e)||e<=0?"uncharted":Ge.find(t=>e<=t.maxRank).tier}function pe(e){return!Number.isFinite(e)||e<=y.zeroAt||e>y.peakTo?0:e>=y.peakFrom?1:T((e-y.zeroAt)/(y.peakFrom-y.zeroAt))}function We(e){if(!Array.isArray(e)||e.length!==5)return null;let t=e.map(o=>Math.max(0,Number(o)||0)),r=t.reduce((o,i)=>o+i,0);if(r<U)return null;let n=t.reduce((o,i,l)=>o+i*Ue[l],0);return T(n/r)}var q=2,Ke=5,Ye=2;function Je(e){if(!Array.isArray(e)||e.length<4)return null;let t=e.map(g=>Math.max(0,Number(g)||0)),r=Math.max(...t);if(r<Ke)return null;let n=t.slice(-q),o=t.slice(-q*2,-q),i=de(n),l=de(o);return Math.max(...n)===r&&i>l*Ye?null:T(ue(i/r))}function fe(e,t=Pe){let r=e.popularity?.edhrecRank,n=S(r);if(!Number.isFinite(r)||r<=0)return{unscored:!0,reason:"No EDHREC rank yet, so obscurity cannot be judged.",tier:n};if(r>w)return{unscored:!0,reason:`Past EDHREC rank ${w}, where the evidence to say this works does not exist.`,tier:n};let o={bracketFit:We(e.bracketCounts),archetypeDepth:e.archetypeDepth===void 0?null:ue(e.archetypeDepth),retention:Je(e.retentionTrend)};if(o.bracketFit===null)return{unscored:!0,reason:E(e),tier:n,...e.bracketCounts?{bracketCounts:e.bracketCounts}:{}};let i=Object.entries(o).filter(([,m])=>m!==null),l=i.reduce((m,[H])=>m+t[H],0),p=T(i.reduce((m,[H,He])=>m+He*t[H],0)/l),g=pe(r);return{tier:n,obscurity:g,worksScore:p,edgeScore:Ve(g*p*100),quality:Object.fromEntries(i),partial:i.length<Object.keys(o).length,reasons:C(e,{obscurity:g,worksScore:p,tier:n,components:o}),modelVersion:qe}}function C(e,{obscurity:t,worksScore:r,tier:n,components:o}={}){return t??=e.obscurity??pe(e.popularity?.edhrecRank),r??=e.worksScore??0,n??=e.tier??S(e.popularity?.edhrecRank),o??={bracketFit:e.quality?.bracketFit??null,archetypeDepth:e.quality?.archetypeDepth??null,retention:e.quality?.retention??null},Ze(e,{obscurity:t,worksScore:r,tier:n,components:o})}function E(e){let t=e.popularity?.edhrecRank;if(!Number.isFinite(t)||t<=0)return"No EDHREC rank yet, so obscurity cannot be judged.";if(t>w)return`Past EDHREC rank ${w.toLocaleString()}, where the evidence to say this works does not exist.`;let r=P(e);return r>0?`Only ${r} bracket-tagged deck${r===1?"":"s"}, below the floor of ${U}.`:"No EDHREC page data has been collected for this commander yet."}function Ze(e,{obscurity:t,worksScore:r,tier:n,components:o}){let i=[];if(t>=1?i.push(`Sits at EDHREC rank ${e.popularity.edhrecRank}, squarely in the Edge tier.`):t>0&&i.push(`Rank ${e.popularity.edhrecRank} is on the edge of the Rare tier, so obscurity counts for less.`),o.bracketFit!==null){let l=Math.round(o.bracketFit*100),p=P(e);i.push(`${l}% of its ${p.toLocaleString()} bracket-tagged decks are built at Bracket 3 or above.`)}else e.bracketCounts&&i.push(`Bracket data shown but not scored: only ${P(e)} tagged decks, below the floor of ${U}.`);return o.archetypeDepth!==null&&i.push(o.archetypeDepth>=.35?"A deep pool of high-synergy cards means there is an archetype here, not just goodstuff.":"A shallow high-synergy pool suggests the deck leans on colour-identity staples."),o.retention!==null&&i.push(o.retention>=.5?"Deck saves are holding or climbing rather than fading after release.":"Deck saves are falling away from their earlier level."),r<.3&&i.push("The evidence that this works is weak, so the Edge score stays low however obscure it is."),n==="rare"&&r>=.5&&i.push("Known enough to have a track record, obscure enough to be worth a second look."),i.slice(0,5)}function P(e){return Array.isArray(e.bracketCounts)?e.bracketCounts.reduce((t,r)=>t+(Number(r)||0),0):0}function de(e){return e.length?e.reduce((t,r)=>t+r,0)/e.length:0}var nr=Object.freeze({cohortPosition:.6,interestToTraction:.4});function A(e){let t=e?.cohort;if(!t)return[];let r=e.popularity?.deckCount??0,n=[`${Xe(t.position)} of ${t.size} new legends in ${String(t.setCode).toUpperCase()}, with ${r.toLocaleString()} deck${r===1?"":"s"} so far.`],o=e.mentionCount;return n.push(Number.isFinite(o)&&o>0?`${o} deck-tech mention${o===1?"":"s"} against ${r.toLocaleString()} build${r===1?"":"s"}.`:"No deck-tech coverage found yet, so this is cohort position alone."),n}function Xe(e){let t=e%100;return t>=11&&t<=13?`${e}th`:`${e}${["th","st","nd","rd"][e%10]??"th"}`}var Qe=/\{([^}]{1,4})\}/g,L=Object.freeze({W:"#f7f2da",U:"#b3dbf2",B:"#c0b5ad",R:"#f2a687",G:"#a4d5ae",C:"#cfd5d3"}),G="#cbcfcd";function et(e){let t=[];for(let[,r]of String(e??"").matchAll(Qe)){let n=tt(r.toUpperCase());n&&t.push(n)}return t}function tt(e){if(e.includes("/")){let t=e.split("/"),r=t.at(-1)==="P",n=(r?t.slice(0,-1):t).filter(i=>L[i]),o=n.length?n.map(i=>L[i]):[G];return r?{glyph:"\u03A6",colors:o}:{glyph:t.map(rt).join(""),colors:o}}return L[e]?{glyph:e,colors:[L[e]]}:/^\d{1,3}$/.test(e)?{glyph:e,colors:[G]}:/^[XYZS]$/.test(e)?{glyph:e,colors:[G]}:null}function rt(e){return/^[WUBRGCXYZS]$|^\d$/.test(e)?e:""}function M(e){let t=String(e??"").split("//").map(et).filter(o=>o.length);if(!t.length)return"";let r=t.map(o=>o.map(nt).join("")).join('<i class="pip-split">//</i>');return`<span class="mana" aria-label="Mana cost ${t.map(at).join(" or ")}" role="img">${r}</span>`}function nt(e){return`<i class="pip-mana" style="background:${ot(e.colors)}">${e.glyph}</i>`}function ot(e){return e.length===1?e[0]:`linear-gradient(-45deg, ${e[0]} 0 50%, ${e.at(-1)} 50% 100%)`}function at(e){return e.map(t=>t.glyph==="\u03A6"?"Phyrexian":t.glyph).join(" ")}var R=`
...
`;var k=document.createElement("div");k.id="mtg-edge-lord-insight";var K=k.attachShadow({mode:"open"});K.innerHTML=`<style>${R}</style><div id="insight"></div>`;var Y=null,he=new Set,V={source:null,bySlug:new Map};function J(e){let t=st(location.pathname);if(!t||!e?.length||he.has(t))return W();let r=it(e).get(t);if(!r)return W();Y!==t&&(K.getElementById("insight").innerHTML=lt(r),K.querySelector("#dismiss")?.addEventListener("click",()=>{he.add(t),W()}),Y=t),k.isConnected||ct()}function it(e){return V.source!==e&&(V={source:e,bySlug:new Map(e.map(t=>[t.slug,t]))}),V.bySlug}function st(e){let[,t,r]=String(e).split("/");return t==="commanders"&&r?decodeURIComponent(r):null}function ct(){let e=document.querySelector("main");e?.parentElement?e.parentElement.insertBefore(k,e):document.body.prepend(k)}function W(){k.remove(),Y=null}function lt(e){let t=e.popularity?.edhrecRank,r=e.popularity?.deckCount;return`<article>
    <div class="head">
    ...
    ${ut(e)}
  </article>`}function dt(e){return e.edgeScore!==void 0?`<span class="score">${e.edgeScore}</span><span class="tier ${pt(e.tier??"")}">${O(e.tier??"scored")}</span>`:e.cohortScore!==void 0?`<span class="score cohort">${e.cohortScore}<small>new</small></span>`:'<span class="score none">\u2014</span>'}function ut(e){return e.cohortScore!==void 0?`<ul class="why">${A(e).map(ge).join("")}</ul>`:e.edgeScore===void 0?`<p class="insufficient">Not scored \u2014 ${O(E(e))}</p>`:`<ul class="why">${C(e).map(ge).join("")}</ul>`}function ge(e){return`<li>${O(e)}</li>`}function O(e){let t=document.createElement("span");return t.textContent=String(e),t.innerHTML}function pt(e){return O(e).replaceAll('"',"&quot;")}var Z="columnar/1";function be(e){if(e?.format!==Z)throw new Error(`Unsupported columnar format: ${e?.format??"missing"}`);let t=Array.from({length:e.count},()=>({}));for(let[r,n]of Object.entries(e.columns)){let o=ft(n,e.count);for(let i=0;i<e.count;i+=1)o[i]!==null&&ht(t[i],r,o[i])}return t}function ft(e,t){if(e.kind==="raw")return e.values;if(e.kind==="tokens")return e.index.map(r=>r===null?null:r.map(n=>e.vocabulary[n]));if(e.kind==="dict")return e.index.map(r=>r===-1?null:me(e.keys[r]));if(e.kind==="sparse"){let r=Array.from({length:t},()=>me(e.fill));return e.index.forEach((n,o)=>{r[n]=e.values[o]}),r}throw new Error(`Unsupported column kind: ${e.kind}`)}function me(e){return typeof e=="object"&&e!==null?structuredClone(e):e}function ht(e,t,r){let n=t.split("."),o=e;for(let i of n.slice(0,-1))gt(o[i])||(o[i]={}),o=o[i];o[n.at(-1)]=r}function gt(e){return typeof e=="object"&&e!==null&&!Array.isArray(e)}var gr=Object.freeze({trendZscore:3,momentum:1,diamondScore:1,edgeScore:1,cohortScore:1,"quality.bracketFit":3,"quality.archetypeDepth":3,"quality.retention":3,price:2});function xe(e){let t=e?.commanders;return Array.isArray(t)?t:t?.format===Z?be(t):[]}var mt="mtg-edge-lord",f="datasets";var N=null;function X(){return N||(N=new Promise(e=>{if(!globalThis.indexedDB)return e(null);let t;try{t=indexedDB.open(mt,1)}catch{return e(null)}t.onupgradeneeded=()=>{t.result.objectStoreNames.contains(f)||t.result.createObjectStore(f)},t.onsuccess=()=>e(t.result),t.onerror=()=>e(null),t.onblocked=()=>e(null)}),N)}async function Q(e){let t=await X();return t?new Promise(r=>{try{let n=t.transaction(f,"readonly").objectStore(f).get(e);n.onsuccess=()=>r(n.result??null),n.onerror=()=>r(null)}catch{r(null)}}):null}async function ee(e,t){let r=await X();return r?new Promise(n=>{try{let o=r.transaction(f,"readwrite");o.objectStore(f).put(t,e),o.oncomplete=()=>n(!0),o.onerror=()=>n(!1),o.onabort=()=>n(!1)}catch{n(!1)}}):!1}async function ye(e){let t=await X();if(t)try{t.transaction(f,"readwrite").objectStore(f).delete(e)}catch{}}var $e="https://rktrobinhood.github.io/MTG-Edge-Lord/data",we="https://raw.githubusercontent.com/RktRobinhood/MTG-Edge-Lord/main/data",ke="datasets:v2",te="commander-detail:v2",bt="mtg-edge-lord:data:v1";async function Se({force:e=!1}={}){yt();let t=await Q(ke);try{let r;try{r=await ve($e,t,e)}catch{r=await ve(we,t,e)}if(r.cached)return{...t,stale:!1};let n={manifest:r.manifest,...r.datasets,cachedAt:new Date().toISOString()};return await ee(ke,n),t?.manifest?.dataVersion!==r.manifest.dataVersion&&await ye(te),{...n,stale:!1}}catch(r){if(t)return{...t,stale:!0,error:r.message};throw r}}async function Ce(e){let t=await Q(te);if(t?.dataVersion===e)return t.detail;for(let r of[$e,we])try{let{detail:n}=await h(`${r}/commander-detail.json`);return await ee(te,{dataVersion:e,detail:n}),n}catch{}return t?.detail??{}}async function ve(e,t,r){let n=await h(`${e}/manifest.json?ts=${Date.now()}`);if(!r&&t?.manifest?.dataVersion===n.dataVersion)return{cached:!0};let[o,i,l,p,g,m]=await Promise.all([h(`${e}/findings.json`),h(`${e}/commanders.json`).then(xt),h(`${e}/hidden-cards.json`),h(`${e}/community-resources.json`),h(`${e}/relationships/card-commander.json`),h(`${e}/archive.json`).catch(()=>({schemaVersion:1,archive:[]}))]);return{manifest:n,datasets:{findings:o,commanders:i,cards:l,resources:p,relationships:g,archive:m}}}function xt(e){return{schemaVersion:e?.schemaVersion??1,commanders:xe(e)}}function yt(){try{localStorage.removeItem(bt)}catch{}}function h(e){let t=globalThis.GM?.xmlHttpRequest??globalThis.GM_xmlhttpRequest;return t?new Promise((r,n)=>t({method:"GET",url:e,headers:{Accept:"application/json"},onload:o=>{if(o.status<200||o.status>=300)return n(new Error(`HTTP ${o.status}`));try{r(JSON.parse(o.responseText))}catch{n(new Error("Backend returned invalid JSON."))}},onerror:()=>n(new Error("Could not reach the MTG Edge Lord data backend."))})):fetch(e).then(kt).then(r=>r.json())}function kt(e){if(!e.ok)throw new Error(`HTTP ${e.status}`);return e}function Ee(e,t=220){let r=String(e??"").trim();if(r.length<=t)return r;let n=r.slice(0,t),o=n.lastIndexOf(" ");return`${(o>t*.6?n.slice(0,o):n).replace(/[\s,;:.]+$/,"")}\u2026`}function vt(e){let t=e?.source?.url;return typeof t=="string"&&/^https:\/\//.test(t)}function Re(e){return(e??[]).filter(vt)}function re(e){let t=String(e?.creator??"").trim(),r=String(e?.name??"").trim();return t&&r&&t.toLowerCase()!==r.toLowerCase()?`${t} \xB7 ${r}`:t||r||"Source"}var $t=["W","U","B","R","G"],I=Object.freeze([{id:"edge",label:"Edge score"},{id:"momentum",label:"Momentum"},{id:"worksScore",label:"How well it works"},{id:"price",label:"Cheapest first"},{id:"released",label:"Most recent"},{id:"name",label:"Alphabetical"},{id:"rank",label:"EDHREC rank"},{id:"deckCount",label:"Deck count (popularity)"}]),Le="edge",Me=Object.freeze([{id:"includes",label:"Includes"},{id:"exact",label:"Exactly"},{id:"atMost",label:"At most"}]),oe=Object.freeze({query:"",colors:[],colorMode:"includes",theme:"",functionalTag:"",creatureType:"",tier:"",minBracketFit:"",minRank:"",maxRank:"",minManaValue:"",maxManaValue:"",maxPrice:"",releasedAfter:"",scoredOnly:!1,sort:Le});function wt(e){return e.map(t=>{if(t.cohortScore!==void 0)return{...t,tier:S(t.popularity?.edhrecRank)};if(t.edgeScore===void 0)return{...t,tier:S(t.popularity?.edhrecRank)};let r=fe(t);if(r.unscored){let n={...t,tier:r.tier};return delete n.edgeScore,n}return{...t,tier:r.tier,obscurity:r.obscurity,worksScore:r.worksScore,edgeScore:r.edgeScore,quality:r.quality}})}function Oe(e){let t=wt(e),r=t.map(n=>[n.name,...n.themes??[],...n.functionalTags??[],...n.creatureTypes??[],...n.types??[]].join(" ").toLowerCase());return{commanders:t,haystacks:r,themes:ne(t,"themes"),functionalTags:ne(t,"functionalTags"),creatureTypes:ne(t,"creatureTypes")}}function ne(e,t){let r=new Map;for(let n of e)for(let o of n[t]??[])r.set(o,(r.get(o)??0)+1);return[...r.entries()].sort((n,o)=>o[1]-n[1]||n[0].localeCompare(o[0])).map(([n])=>n)}function Ne(e,t){let r=String(t.query??"").toLowerCase().split(/\s+/).filter(Boolean),n=[];for(let o=0;o<e.commanders.length;o+=1){if(r.length&&!r.every(l=>e.haystacks[o].includes(l)))continue;let i=e.commanders[o];St(i,t)&&n.push(i)}return n}function St(e,t){let r=e.popularity?.edhrecRank;return!(!Ct(e.colorIdentity,t.colors,t.colorMode)||t.theme&&!(e.themes??[]).includes(t.theme)||t.functionalTag&&!(e.functionalTags??[]).includes(t.functionalTag)||t.creatureType&&!(e.creatureTypes??[]).includes(t.creatureType)||t.tier&&e.tier!==t.tier||t.scoredOnly&&e.edgeScore===void 0||t.minBracketFit!==""&&!(e.quality?.bracketFit>=Number(t.minBracketFit))||!Te(r,t.minRank,t.maxRank)||!Te(e.manaValue,t.minManaValue,t.maxManaValue)||t.maxPrice!==""&&e.price!==void 0&&e.price>Number(t.maxPrice)||t.releasedAfter&&(e.releasedAt??"")<t.releasedAfter)}function Te(e,t,r){return t===""&&r===""?!0:!(!Number.isFinite(e)||t!==""&&e<Number(t)||r!==""&&e>Number(r))}function Ct(e,t,r){if(!t?.length)return!0;if(e===void 0)return!1;let n=$t.filter(o=>t.includes(o));return r==="exact"?e===n.join(""):r==="atMost"?[...e].every(o=>n.includes(o)):n.every(o=>e.includes(o))}function je(e,t){let r=Ae[t]??Ae[Le];return[...e].sort(r)}var j=e=>(t,r)=>{let n=e(t),o=e(r);return n===void 0&&o===void 0?v(t,r):n===void 0?1:o===void 0?-1:o-n||v(t,r)},Et=e=>(t,r)=>{let n=e(t),o=e(r);return n===void 0&&o===void 0?v(t,r):n===void 0?1:o===void 0?-1:n-o||v(t,r)};function v(e,t){return(e.popularity?.edhrecRank??1/0)-(t.popularity?.edhrecRank??1/0)}var Ae={edge:j(e=>e.edgeScore),momentum:j(e=>e.momentum),worksScore:j(e=>e.worksScore),price:Et(e=>e.price),released:(e,t)=>String(t.releasedAt??"").localeCompare(String(e.releasedAt??""))||v(e,t),name:(e,t)=>e.name.localeCompare(t.name),rank:v,deckCount:j(e=>e.popularity?.deckCount)};var x=60,Rt=140,_e="mtg-edge-lord:filters",Tt=[{id:"W",label:"W"},{id:"U",label:"U"},{id:"B",label:"B"},{id:"R",label:"R"},{id:"G",label:"G"}],At=[{id:"",label:"Any tier"},{id:"edge",label:"Edge (1,000\u20133,000)"},{id:"rare",label:"Rare (500\u20131,000)"},{id:"meta",label:"Meta (top 500)"},{id:"uncharted",label:"Uncharted (3,000+)"}],a={tab:"search",filters:{...oe,...Xt()},shown:x,expanded:null,detail:null,index:null,data:null,loading:!0,error:null},Ie=null,ce=document.createElement("div");ce.id="mtg-edge-lord-root";var u=ce.attachShadow({mode:"open"}),d=document.createElement("div");d.id="mtg-edge-lord-button";var se=d.attachShadow({mode:"open"});document.body.append(ce);Lt();Fe();Ot();jt();function Lt(){se.innerHTML=`<style>${R}</style>
    ...
    <section id="panel" aria-label="MTG Edge Lord" hidden>
      ...
      <main></main>
    </section>`,se.getElementById("toggle").addEventListener("click",Nt),u.querySelector("nav").addEventListener("click",e=>{let t=e.target.closest("button[data-tab]");t&&(a.tab=t.dataset.tab,a.shown=x,le())})}function Mt(){for(let e of document.querySelectorAll('header nav input[aria-label="Search"], header nav input.rbt-input-main')){let t=e.closest('[class*="Navbar_search"], .input-group');if(t?.getBoundingClientRect().width)return t}return null}function Fe(){let e=Mt();if(e){d.classList.remove("floating"),d.previousElementSibling!==e&&e.after(d);return}d.classList.add("floating"),d.parentElement!==document.body&&document.body.append(d)}function Ot(){let e=null,t=()=>{clearTimeout(e),e=setTimeout(()=>{Fe(),J(a.index?.commanders),u.getElementById("panel").hidden||Be()},100)};new MutationObserver(t).observe(document.body,{childList:!0,subtree:!0}),addEventListener("resize",t)}function Nt(){let e=u.getElementById("panel"),t=e.hidden;t&&Be(),e.hidden=!t;let r=se.getElementById("toggle");r.setAttribute("aria-expanded",String(t)),r.classList.toggle("on",t)}var De=14;function Be(){let e=u.getElementById("panel"),t=d.getBoundingClientRect(),r=!d.classList.contains("floating");e.style.setProperty("--mel-top",`${r?Math.round(t.bottom)+8:De}px`),e.style.setProperty("--mel-right",`${De}px`)}async function jt(){try{a.data=await Se(),a.index=Oe(a.data.commanders.commanders)}catch(e){a.error=e.message}finally{a.loading=!1,le(),J(a.index?.commanders)}}function le(){u.querySelectorAll("nav button").forEach(t=>t.classList.toggle("active",t.dataset.tab===a.tab)),u.getElementById("status").textContent=It();let e=u.querySelector("main");if(a.loading)return void(e.innerHTML='<div class="empty">Loading\u2026</div>');if(a.error)return void(e.innerHTML=`<div class="empty">${s(a.error)}</div>`);e.innerHTML=`
    ...
    <div class="stack" id="results"></div>`,Dt(e),$()}function It(){return a.data?`${(a.index?.commanders.length??0).toLocaleString()} commanders \xB7 ${a.data.manifest.dataVersion}${a.data.stale?" \xB7 cached":""}`:"Discovery data unavailable"}function Dt(e){e.querySelectorAll("[data-field]").forEach(t=>{let r=t.tagName==="SELECT"||t.type==="checkbox"?"change":"input";t.addEventListener(r,()=>{let n=t.type==="checkbox"?t.checked:t.value;a.filters[t.dataset.field]=n,a.shown=x,ie(),r==="input"?_t():$()})}),e.querySelectorAll("[data-color]").forEach(t=>t.addEventListener("click",()=>{let r=t.dataset.color,n=a.filters.colors.includes(r)?a.filters.colors.filter(o=>o!==r):[...a.filters.colors,r];a.filters.colors=n,t.classList.toggle("on",n.includes(r)),a.shown=x,ie(),$()})),e.addEventListener("click",t=>{if(t.target.closest("a, button, input, select, label"))return;let r=t.target.closest("[data-open]");r&&open(r.dataset.open,"_blank","noopener")}),e.querySelector("#reset")?.addEventListener("click",()=>{a.filters={...oe},a.shown=x,ie(),le()})}function _t(){clearTimeout(Ie),Ie=setTimeout($,Rt)}function $(){let e=u.getElementById("results"),t=u.getElementById("result-summary");if(!e)return;if(a.tab==="discover"){let o=Re(a.data.findings.findings).filter(i=>ae(`${i.title} ${i.summary}`,a.filters.query)).sort((i,l)=>l.score.total-i.score.total);return t.textContent=`${o.length} find${o.length===1?"":"s"}`,e.innerHTML=o.slice(0,a.shown).map(Yt).join("")||F("No finds match that search yet."),D(e)}if(a.tab==="archive"){let o=(a.data.archive?.archive??[]).filter(i=>ae(i.name,a.filters.query));return t.textContent=o.length?`${o.length} commander${o.length===1?"":"s"} surfaced to date`:"Nothing surfaced yet",e.innerHTML=o.slice(0,a.shown).map(Jt).join("")||F("No commander has been surfaced under that name yet."),D(e)}if(a.tab==="card"){let o=a.data.cards.cards.filter(i=>ae(i.name,a.filters.query));return t.textContent=`${o.length} card${o.length===1?"":"s"}`,e.innerHTML=o.slice(0,a.shown).map(Zt).join("")||F("No cards match that search yet."),D(e)}let r=je(Ne(a.index,a.filters),a.filters.sort),n=r.filter(o=>o.edgeScore!==void 0).length;t.innerHTML=`<strong>${r.length.toLocaleString()}</strong> commanders \xB7 ${n.toLocaleString()} scored \xB7 sorted by ${s(Qt(a.filters.sort))}`,e.innerHTML=r.slice(0,a.shown).map(zt).join("")||F("Nothing matches those filters. Widen the rank band or clear a colour."),r.length>a.shown&&e.insertAdjacentHTML("beforeend",`<button class="more" id="more" type="button">Show ${Math.min(x,r.length-a.shown)} more of ${(r.length-a.shown).toLocaleString()}</button>`),D(e)}function D(e){e.querySelector("#more")?.addEventListener("click",()=>{a.shown+=x,$()}),e.querySelectorAll("[data-expand]").forEach(t=>t.addEventListener("click",async()=>{let r=t.dataset.expand;a.expanded=a.expanded===r?null:r,a.expanded&&!a.detail&&(a.detail=await Ce(a.data.manifest.dataVersion)),$()}))}function Ft(){let e=a.filters;return`<div class="controls">
    ...
    <div class="row colors">
      ...
      ${Tt.map(t=>`<button type="button" class="pip ${e.colors.includes(t.id)?"on":""}" data-color="${t.id}" aria-label="${t.label}">${t.label}</button>`).join("")}
    ...
    <div class="row">
      ...
      <select data-field="tier" aria-label="Tier">${b(At.map(t=>[t.id,t.label]),e.tier)}</select>
    </div>
    <div class="row">
  ...
  </div>`}function Bt(){return`<div class="controls"><input class="query" data-field="query" value="${c(a.filters.query)}" placeholder="Search" aria-label="Search"></div>`}function zt(e){let t=a.expanded===e.slug,r=B(e.slug),n=(e.themes??[]).slice(0,3);return`<article class="card commander" data-open="${c(r)}">
    <div class="head">
    ...
    ${t?Pt(e):""}
  </article>`}function B(e){return`https://edhrec.com/commanders/${encodeURIComponent(e)}`}function Ht(e){let t=M(e.manaCost);return t||(e.colorIdentity===void 0?"":`<span class="chip">${s(e.colorIdentity||"Colourless")}</span>`)}function qt(e){let t=e.popularity?.edhrecRank,r=e.popularity?.deckCount;return[t?`#${t.toLocaleString()}`:null,r?`${r.toLocaleString()} decks`:null,e.price!==void 0?`$${e.price.toFixed(2)}`:null].filter(Boolean).join(" \xB7 ")}function Pt(e){return`<div class="detail">
    ${Gt(e)}
    ${Vt(e)}
    ${Wt(e)}
  </div>`}function Ut(e){return e.edgeScore!==void 0?`<span class="score">${e.edgeScore}</span>`:e.cohortScore!==void 0?`<span class="score cohort" title="Scored against its set cohort, not the whole format">${e.cohortScore}<small>new</small></span>`:'<span class="score none" title="Not enough evidence to score">\u2014</span>'}function Gt(e){return e.cohortScore!==void 0?`<p class="cohort-note">New arrival \u2014 scored against its ${s(String(e.cohort?.setCode??"").toUpperCase())} set cohort, not the whole format.</p>
      <ul class="why">${A(e).map(t=>`<li>${s(t)}</li>`).join("")}</ul>`:e.edgeScore===void 0?`<p class="insufficient">Insufficient data \u2014 ${s(E(e))}</p>`:`<ul class="why">${C(e).map(t=>`<li>${s(t)}</li>`).join("")}</ul>`}function Vt(e){let t=e.quality;if(!t)return"";let r=[["Bracket fit",t.bracketFit],["Archetype depth",t.archetypeDepth],["Retention",t.retention]].filter(([,n])=>n!==void 0);return r.length?`<div class="chips">${r.map(([n,o])=>`<span class="chip quality">${n} ${Math.round(o*100)}%</span>`).join("")}${e.partialScore?'<span class="chip muted">partial</span>':""}</div>`:""}function Wt(e){let t=a.detail?.[e.slug];return t?`<div class="more-detail">
  ...
  </article>`}function Zt(e){let t=e.commanders.map(r=>`<a class="chip" href="${c(B(r.slug))}" target="_blank" rel="noopener noreferrer">${s(r.name)} \xB7 ${r.relationshipScore}</a>`).join("");return`<article class="card"><h3>${s(e.name)}</h3><div class="chips">${t}</div>${ze(e.findingIds)}</article>`}function ze(e=[]){let t=new Set(e),r=(a.data.resources.resources??[]).filter(n=>t.has(n.findingId)&&typeof n.url=="string"&&n.url.startsWith("https://")).slice(0,3);return r.length?`<p class="sources">${r.map(n=>`<a href="${c(n.url)}" target="_blank" rel="noopener noreferrer">${s(n.creator)} \xB7 ${er(n.resourceDepth)} \u2197</a>`).join(" \xB7 ")}</p>`:""}function ae(e,t){let r=String(t??"").trim().toLowerCase();return!r||String(e).toLowerCase().includes(r)}function ie(){try{sessionStorage.setItem(_e,JSON.stringify(a.filters))}catch{}}function Xt(){try{return JSON.parse(sessionStorage.getItem(_e))??{}}catch{return{}}}function Qt(e){return I.find(t=>t.id===e)?.label??I[0].label}function z(e){return String(e).replaceAll("-"," ").replace(/\b[a-z]/g,t=>t.toUpperCase())}function b(e,t){return e.map(([r,n])=>`<option value="${c(r)}" ${String(t)===String(r)?"selected":""}>${s(n)}</option>`).join("")}function _(e,t){return`<button type="button" data-tab="${e}" class="${a.tab===e?"active":""}">${t}</button>`}function er(e){return s(String(e).replaceAll("_"," "))}function F(e){return`<div class="empty">${s(e)}</div>`}function s(e){let t=document.createElement("span");return t.textContent=String(e),t.innerHTML}function c(e){return s(e).replaceAll('"',"&quot;")}})();

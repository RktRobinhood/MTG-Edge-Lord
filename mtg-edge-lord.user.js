...
`;var g=60,Ye=140,$e="mtg-edge-lord:filters",Je=[{id:"W",label:"W"},{id:"U",label:"U"},{id:"B",label:"B"},{id:"R",label:"R"},{id:"G",label:"G"}],Ze=[{id:"",label:"Any tier"},{id:"edge",label:"Edge (1,000\u20133,000)"},{id:"rare",label:"Rare (500\u20131,000)"},{id:"meta",label:"Meta (top 500)"},{id:"uncharted",label:"Uncharted (3,000+)"}],a={tab:"search",filters:{...B,...ft()},shown:g,expanded:null,detail:null,index:null,data:null,loading:!0,error:null},xe=null,G=document.createElement("div");G.id="mtg-edge-lord-root";var d=G.attachShadow({mode:"open"});document.body.append(G);Xe();Qe();function Xe(){d.innerHTML=`<style>${ye}</style>
    ...
    <section id="panel" aria-label="MTG Edge Lord" hidden>
      ...
      <nav>${U("search","Search")}${U("discover","Recent finds")}${U("card","Card-first")}</nav>
      <main></main>
  ...
  </a>`}function pt(e){let t=e.commanders.map(r=>`<a class="chip" href="https://edhrec.com/commanders/${encodeURIComponent(r.slug)}" target="_blank" rel="noopener noreferrer">${i(r.name)} \xB7 ${r.relationshipScore}</a>`).join("");return`<article class="card"><h3>${i(e.name)}</h3><div class="chips">${t}</div>${we(e.findingIds)}</article>`}function we(e=[]){let t=new Set(e),r=(a.data.resources.resources??[]).filter(n=>t.has(n.findingId)&&typeof n.url=="string"&&n.url.startsWith("https://")).slice(0,3);return r.length?`<p class="sources">${r.map(n=>`<a href="${c(n.url)}" target="_blank" rel="noopener noreferrer">${i(n.creator)} \xB7 ${mt(n.resourceDepth)} \u2197</a>`).join(" \xB7 ")}</p>`:""}function ke(e,t){let r=String(t??"").trim().toLowerCase();return!r||String(e).toLowerCase().includes(r)}function P(){try{sessionStorage.setItem($e,JSON.stringify(a.filters))}catch{}}function ft(){try{return JSON.parse(sessionStorage.getItem($e))??{}}catch{return{}}}function ht(e){return E.find(t=>t.id===e)?.label??E[0].label}function R(e){return String(e).replaceAll("-"," ").replace(/\b[a-z]/g,t=>t.toUpperCase())}function m(e,t){return e.map(([r,n])=>`<option value="${c(r)}" ${String(t)===String(r)?"selected":""}>${i(n)}</option>`).join("")}function U(e,t){return`<button type="button" data-tab="${e}" class="${a.tab===e?"active":""}">${t}</button>`}function mt(e){return i(String(e).replaceAll("_"," "))}function V(e){return`<div class="empty">${i(e)}</div>`}function i(e){let t=document.createElement("span");return t.textContent=String(e),t.innerHTML}function c(e){return i(e).replaceAll('"',"&quot;")}})();

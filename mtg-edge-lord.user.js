...
// ==/UserScript==
(()=>{var Je="edge-v2",Ze=Object.freeze({bracketFit:.4,archetypeDepth:.4,retention:.2}),Xe=Object.freeze([0,0,1,1,.6]),G=30,x=Object.freeze({zeroAt:500,peakFrom:1e3,peakTo:3e3}),$=3e3,Qe=Object.freeze([{tier:"meta",maxRank:500},{tier:"rare",maxRank:1e3},{tier:"edge",maxRank:3e3},{tier:"uncharted",maxRank:1/0}]),et=e=>Math.round(e*10)/10,A=e=>Math.round(e*1e3)/1e3,fe=e=>Math.max(0,Math.min(1,Number(e)||0));function S(e){return!Number.isFinite(e)||e<=0?"uncharted":Qe.find(t=>e<=t.maxRank).tier}function he(e){return!Number.isFinite(e)||e<=x.zeroAt||e>x.peakTo?0:e>=x.peakFrom?1:A((e-x.zeroAt)/(x.peakFrom-x.zeroAt))}function tt(e){if(!Array.isArray(e)||e.length!==5)return null;let t=e.map(r=>Math.max(0,Number(r)||0)),n=t.reduce((r,i)=>r+i,0);if(n<G)return null;let o=t.reduce((r,i,l)=>r+i*Xe[l],0);return A(o/n)}var P=2,nt=5,rt=2;function ot(e){if(!Array.isArray(e)||e.length<4)return null;let t=e.map(g=>Math.max(0,Number(g)||0)),n=Math.max(...t);if(n<nt)return null;let o=t.slice(-P),r=t.slice(-P*2,-P),i=pe(o),l=pe(r);return Math.max(...o)===n&&i>l*rt?null:A(fe(i/n))}function ge(e,t=Ze){let n=e.popularity?.edhrecRank,o=S(n);if(!Number.isFinite(n)||n<=0)return{unscored:!0,reason:"No EDHREC rank yet, so obscurity cannot be judged.",tier:o};if(n>$)return{unscored:!0,reason:`Past EDHREC rank ${$}, where the evidence to say this works does not exist.`,tier:o};let r={bracketFit:tt(e.bracketCounts),archetypeDepth:e.archetypeDepth===void 0?null:fe(e.archetypeDepth),retention:ot(e.retentionTrend)};if(r.bracketFit===null)return{unscored:!0,reason:E(e),tier:o,...e.bracketCounts?{bracketCounts:e.bracketCounts}:{}};let i=Object.entries(r).filter(([,m])=>m!==null),l=i.reduce((m,[q])=>m+t[q],0),u=A(i.reduce((m,[q,Ye])=>m+Ye*t[q],0)/l),g=he(n);return{tier:o,obscurity:g,worksScore:u,edgeScore:et(g*u*100),quality:Object.fromEntries(i),partial:i.length<Object.keys(r).length,reasons:C(e,{obscurity:g,worksScore:u,tier:o,components:r}),modelVersion:Je}}function C(e,{obscurity:t,worksScore:n,tier:o,components:r}={}){return t??=e.obscurity??he(e.popularity?.edhrecRank),n??=e.worksScore??0,o??=e.tier??S(e.popularity?.edhrecRank),r??={bracketFit:e.quality?.bracketFit??null,archetypeDepth:e.quality?.archetypeDepth??null,retention:e.quality?.retention??null},at(e,{obscurity:t,worksScore:n,tier:o,components:r})}function E(e){let t=e.popularity?.edhrecRank;if(!Number.isFinite(t)||t<=0)return"No EDHREC rank yet, so obscurity cannot be judged.";if(t>$)return`Past EDHREC rank ${$.toLocaleString()}, where the evidence to say this works does not exist.`;let n=U(e);return n>0?`Only ${n} bracket-tagged deck${n===1?"":"s"}, below the floor of ${G}.`:"No EDHREC page data has been collected for this commander yet."}function at(e,{obscurity:t,worksScore:n,tier:o,components:r}){let i=[];if(t>=1?i.push(`Sits at EDHREC rank ${e.popularity.edhrecRank}, squarely in the Edge tier.`):t>0&&i.push(`Rank ${e.popularity.edhrecRank} is on the edge of the Rare tier, so obscurity counts for less.`),r.bracketFit!==null){let l=Math.round(r.bracketFit*100),u=U(e);i.push(`${l}% of its ${u.toLocaleString()} bracket-tagged decks are built at Bracket 3 or above.`)}else e.bracketCounts&&i.push(`Bracket data shown but not scored: only ${U(e)} tagged decks, below the floor of ${G}.`);return r.archetypeDepth!==null&&i.push(r.archetypeDepth>=.35?"A deep pool of high-synergy cards means there is an archetype here, not just goodstuff.":"A shallow high-synergy pool suggests the deck leans on colour-identity staples."),r.retention!==null&&i.push(r.retention>=.5?"Deck saves are holding or climbing rather than fading after release.":"Deck saves are falling away from their earlier level."),n<.3&&i.push("The evidence that this works is weak, so the Edge score stays low however obscure it is."),o==="rare"&&n>=.5&&i.push("Known enough to have a track record, obscure enough to be worth a second look."),i.slice(0,5)}function U(e){return Array.isArray(e.bracketCounts)?e.bracketCounts.reduce((t,n)=>t+(Number(n)||0),0):0}function pe(e){return e.length?e.reduce((t,n)=>t+n,0)/e.length:0}var fn=Object.freeze({cohortPosition:.6,interestToTraction:.4});function T(e){let t=e?.cohort;if(!t)return[];let n=e.popularity?.deckCount??0,o=[`${it(t.position)} of ${t.size} new legends in ${String(t.setCode).toUpperCase()}, with ${n.toLocaleString()} deck${n===1?"":"s"} so far.`],r=e.mentionCount;return o.push(Number.isFinite(r)&&r>0?`${r} deck-tech mention${r===1?"":"s"} against ${n.toLocaleString()} build${n===1?"":"s"}.`:"No deck-tech coverage found yet, so this is cohort position alone."),o}function it(e){let t=e%100;return t>=11&&t<=13?`${e}th`:`${e}${["th","st","nd","rd"][e%10]??"th"}`}var st=/\{([^}]{1,4})\}/g,L=Object.freeze({W:"#f7f2da",U:"#b3dbf2",B:"#c0b5ad",R:"#f2a687",G:"#a4d5ae",C:"#cfd5d3"}),V="#cbcfcd";function ct(e){let t=[];for(let[,n]of String(e??"").matchAll(st)){let o=lt(n.toUpperCase());o&&t.push(o)}return t}function lt(e){if(e.includes("/")){let t=e.split("/"),n=t.at(-1)==="P",o=(n?t.slice(0,-1):t).filter(i=>L[i]),r=o.length?o.map(i=>L[i]):[V];return n?{glyph:"\u03A6",colors:r}:{glyph:t.map(dt).join(""),colors:r}}return L[e]?{glyph:e,colors:[L[e]]}:/^\d{1,3}$/.test(e)?{glyph:e,colors:[V]}:/^[XYZS]$/.test(e)?{glyph:e,colors:[V]}:null}function dt(e){return/^[WUBRGCXYZS]$|^\d$/.test(e)?e:""}function M(e){let t=String(e??"").split("//").map(ct).filter(r=>r.length);if(!t.length)return"";let n=t.map(r=>r.map(ut).join("")).join('<i class="pip-split">//</i>');return`<span class="mana" aria-label="Mana cost ${t.map(ft).join(" or ")}" role="img">${n}</span>`}function ut(e){return`<i class="pip-mana" style="background:${pt(e.colors)}">${e.glyph}</i>`}function pt(e){return e.length===1?e[0]:`linear-gradient(-45deg, ${e[0]} 0 50%, ${e.at(-1)} 50% 100%)`}function ft(e){return e.map(t=>t.glyph==="\u03A6"?"Phyrexian":t.glyph).join(" ")}var R=`
...
`;var y=document.createElement("div");y.id="mtg-edge-lord-insight";var Y=y.attachShadow({mode:"open"});Y.innerHTML=`<style>${R}</style><div id="insight"></div>`;var J=null,me=new Set,W={source:null,bySlug:new Map};function Z(e){let t=gt(location.pathname);if(!t||!e?.length||me.has(t))return K();let n=ht(e).get(t);if(!n)return K();J!==t&&(Y.getElementById("insight").innerHTML=bt(n),Y.querySelector("#dismiss")?.addEventListener("click",()=>{me.add(t),K()}),J=t),y.isConnected||mt()}function ht(e){return W.source!==e&&(W={source:e,bySlug:new Map(e.map(t=>[t.slug,t]))}),W.bySlug}function gt(e){let[,t,n]=String(e).split("/");return t==="commanders"&&n?decodeURIComponent(n):null}function mt(){let e=document.querySelector("main");e?.parentElement?e.parentElement.insertBefore(y,e):document.body.prepend(y)}function K(){y.remove(),J=null}function bt(e){let t=e.popularity?.edhrecRank,n=e.popularity?.deckCount;return`<article>
    <div class="head">
    ...
    ${yt(e)}
  </article>`}function xt(e){return e.edgeScore!==void 0?`<span class="score">${e.edgeScore}</span><span class="tier ${kt(e.tier??"")}">${O(e.tier??"scored")}</span>`:e.cohortScore!==void 0?`<span class="score cohort">${e.cohortScore}<small>new</small></span>`:'<span class="score none">\u2014</span>'}function yt(e){return e.cohortScore!==void 0?`<ul class="why">${T(e).map(be).join("")}</ul>`:e.edgeScore===void 0?`<p class="insufficient">Not scored \u2014 ${O(E(e))}</p>`:`<ul class="why">${C(e).map(be).join("")}</ul>`}function be(e){return`<li>${O(e)}</li>`}function O(e){let t=document.createElement("span");return t.textContent=String(e),t.innerHTML}function kt(e){return O(e).replaceAll('"',"&quot;")}var X="columnar/1";function ye(e){if(e?.format!==X)throw new Error(`Unsupported columnar format: ${e?.format??"missing"}`);let t=Array.from({length:e.count},()=>({}));for(let[n,o]of Object.entries(e.columns)){let r=vt(o,e.count);for(let i=0;i<e.count;i+=1)r[i]!==null&&wt(t[i],n,r[i])}return t}function vt(e,t){if(e.kind==="raw")return e.values;if(e.kind==="tokens")return e.index.map(n=>n===null?null:n.map(o=>e.vocabulary[o]));if(e.kind==="dict")return e.index.map(n=>n===-1?null:xe(e.keys[n]));if(e.kind==="sparse"){let n=Array.from({length:t},()=>xe(e.fill));return e.index.forEach((o,r)=>{n[o]=e.values[r]}),n}throw new Error(`Unsupported column kind: ${e.kind}`)}function xe(e){return typeof e=="object"&&e!==null?structuredClone(e):e}function wt(e,t,n){let o=t.split("."),r=e;for(let i of o.slice(0,-1))$t(r[i])||(r[i]={}),r=r[i];r[o.at(-1)]=n}function $t(e){return typeof e=="object"&&e!==null&&!Array.isArray(e)}var Cn=Object.freeze({trendZscore:3,momentum:1,diamondScore:1,edgeScore:1,cohortScore:1,"quality.bracketFit":3,"quality.archetypeDepth":3,"quality.retention":3,price:2});function ke(e){let t=e?.commanders;return Array.isArray(t)?t:t?.format===X?ye(t):[]}var St="mtg-edge-lord",f="datasets";var N=null;function Q(){return N||(N=new Promise(e=>{if(!globalThis.indexedDB)return e(null);let t;try{t=indexedDB.open(St,1)}catch{return e(null)}t.onupgradeneeded=()=>{t.result.objectStoreNames.contains(f)||t.result.createObjectStore(f)},t.onsuccess=()=>e(t.result),t.onerror=()=>e(null),t.onblocked=()=>e(null)}),N)}async function ee(e){let t=await Q();return t?new Promise(n=>{try{let o=t.transaction(f,"readonly").objectStore(f).get(e);o.onsuccess=()=>n(o.result??null),o.onerror=()=>n(null)}catch{n(null)}}):null}async function D(e,t){let n=await Q();return n?new Promise(o=>{try{let r=n.transaction(f,"readwrite");r.objectStore(f).put(t,e),r.oncomplete=()=>o(!0),r.onerror=()=>o(!1),r.onabort=()=>o(!1)}catch{o(!1)}}):!1}async function ve(e){let t=await Q();if(t)try{t.transaction(f,"readwrite").objectStore(f).delete(e)}catch{}}var $e="https://rktrobinhood.github.io/MTG-Edge-Lord/data",Se="https://raw.githubusercontent.com/RktRobinhood/MTG-Edge-Lord/main/data",te="datasets:v2",ne="commander-detail:v2",Ct=1440*60*1e3;function Et(e,t){let n=t-Date.parse(e?.checkedAt);return!(n>=0&&n<Ct)}var Rt="mtg-edge-lord:data:v1";async function Ce({force:e=!1,checkRemote:t=!0}={}){Tt();let n=await ee(te),o=Date.now();if(n&&!e&&(!t||!Et(n,o)))return{...n,stale:!1};try{let r;try{r=await we($e,n,e)}catch{r=await we(Se,n,e)}let i=new Date(o).toISOString();if(r.cached){let u={...n,checkedAt:i};return await D(te,u),{...u,stale:!1}}let l={manifest:r.manifest,...r.datasets,cachedAt:new Date(o).toISOString(),checkedAt:i};return await D(te,l),n?.manifest?.dataVersion!==r.manifest.dataVersion&&await ve(ne),{...l,stale:!1}}catch(r){if(n)return{...n,stale:!0,error:r.message};throw r}}async function Ee(e){let t=await ee(ne);if(t?.dataVersion===e)return t.detail;for(let n of[$e,Se])try{let{detail:o}=await h(`${n}/commander-detail.json`);return await D(ne,{dataVersion:e,detail:o}),o}catch{}return t?.detail??{}}async function we(e,t,n){let o=await h(`${e}/manifest.json?ts=${Date.now()}`);if(!n&&t?.manifest?.dataVersion===o.dataVersion)return{cached:!0};let[r,i,l,u,g,m]=await Promise.all([h(`${e}/findings.json`),h(`${e}/commanders.json`).then(At),h(`${e}/hidden-cards.json`),h(`${e}/community-resources.json`),h(`${e}/relationships/card-commander.json`),h(`${e}/archive.json`).catch(()=>({schemaVersion:1,archive:[]}))]);return{manifest:o,datasets:{findings:r,commanders:i,cards:l,resources:u,relationships:g,archive:m}}}function At(e){return{schemaVersion:e?.schemaVersion??1,commanders:ke(e)}}function Tt(){try{localStorage.removeItem(Rt)}catch{}}function h(e){let t=globalThis.GM?.xmlHttpRequest??globalThis.GM_xmlhttpRequest;return t?new Promise((n,o)=>t({method:"GET",url:e,headers:{Accept:"application/json"},onload:r=>{if(r.status<200||r.status>=300)return o(new Error(`HTTP ${r.status}`));try{n(JSON.parse(r.responseText))}catch{o(new Error("Backend returned invalid JSON."))}},onerror:()=>o(new Error("Could not reach the MTG Edge Lord data backend."))})):fetch(e).then(Lt).then(n=>n.json())}function Lt(e){if(!e.ok)throw new Error(`HTTP ${e.status}`);return e}function Te(e,t=220){let n=String(e??"").trim();if(n.length<=t)return n;let o=n.slice(0,t),r=o.lastIndexOf(" ");return`${(r>t*.6?o.slice(0,r):o).replace(/[\s,;:.]+$/,"")}\u2026`}function Mt(e){let t=e?.source?.url;return typeof t=="string"&&/^https:\/\//.test(t)}function Le(e){return(e??[]).filter(Mt)}function re(e){let t=String(e?.creator??"").trim(),n=String(e?.name??"").trim();return t&&n&&t.toLowerCase()!==n.toLowerCase()?`${t} \xB7 ${n}`:t||n||"Source"}var Me=10;function Oe(e){return[...e??[]].sort((t,n)=>Re(n.observedAt).localeCompare(Re(t.observedAt))||Ae(n.publishedAt)-Ae(t.publishedAt)||(n.score?.total??0)-(t.score?.total??0))}function Re(e){return String(e??"").slice(0,10)}function Ae(e){let t=Date.parse(e);return Number.isNaN(t)?-1/0:t}var Ot=["W","U","B","R","G"],_=Object.freeze([{id:"edge",label:"Edge score"},{id:"momentum",label:"Momentum"},{id:"worksScore",label:"How well it works"},{id:"price",label:"Cheapest first"},{id:"released",label:"Most recent"},{id:"name",label:"Alphabetical"},{id:"rank",label:"EDHREC rank"},{id:"deckCount",label:"Deck count (popularity)"}]),Ie="edge",_e=Object.freeze([{id:"includes",label:"Includes"},{id:"exact",label:"Exactly"},{id:"atMost",label:"At most"}]),ae=Object.freeze({query:"",colors:[],colorMode:"includes",theme:"",functionalTag:"",creatureType:"",tier:"",minBracketFit:"",minRank:"",maxRank:"",minManaValue:"",maxManaValue:"",maxPrice:"",releasedAfter:"",scoredOnly:!1,sort:Ie});function Nt(e){return e.map(t=>{if(t.cohortScore!==void 0)return{...t,tier:S(t.popularity?.edhrecRank)};if(t.edgeScore===void 0)return{...t,tier:S(t.popularity?.edhrecRank)};let n=ge(t);if(n.unscored){let o={...t,tier:n.tier};return delete o.edgeScore,o}return{...t,tier:n.tier,obscurity:n.obscurity,worksScore:n.worksScore,edgeScore:n.edgeScore,quality:n.quality}})}function je(e){let t=Nt(e),n=t.map(o=>[o.name,...o.themes??[],...o.functionalTags??[],...o.creatureTypes??[],...o.types??[]].join(" ").toLowerCase());return{commanders:t,haystacks:n,themes:oe(t,"themes"),functionalTags:oe(t,"functionalTags"),creatureTypes:oe(t,"creatureTypes")}}function oe(e,t){let n=new Map;for(let o of e)for(let r of o[t]??[])n.set(r,(n.get(r)??0)+1);return[...n.entries()].sort((o,r)=>r[1]-o[1]||o[0].localeCompare(r[0])).map(([o])=>o)}function Fe(e,t){let n=String(t.query??"").toLowerCase().split(/\s+/).filter(Boolean),o=[];for(let r=0;r<e.commanders.length;r+=1){if(n.length&&!n.every(l=>e.haystacks[r].includes(l)))continue;let i=e.commanders[r];Dt(i,t)&&o.push(i)}return o}function Dt(e,t){let n=e.popularity?.edhrecRank;return!(!It(e.colorIdentity,t.colors,t.colorMode)||t.theme&&!(e.themes??[]).includes(t.theme)||t.functionalTag&&!(e.functionalTags??[]).includes(t.functionalTag)||t.creatureType&&!(e.creatureTypes??[]).includes(t.creatureType)||t.tier&&e.tier!==t.tier||t.scoredOnly&&e.edgeScore===void 0||t.minBracketFit!==""&&!(e.quality?.bracketFit>=Number(t.minBracketFit))||!Ne(n,t.minRank,t.maxRank)||!Ne(e.manaValue,t.minManaValue,t.maxManaValue)||t.maxPrice!==""&&e.price!==void 0&&e.price>Number(t.maxPrice)||t.releasedAfter&&(e.releasedAt??"")<t.releasedAfter)}function Ne(e,t,n){return t===""&&n===""?!0:!(!Number.isFinite(e)||t!==""&&e<Number(t)||n!==""&&e>Number(n))}function It(e,t,n){if(!t?.length)return!0;if(e===void 0)return!1;let o=Ot.filter(r=>t.includes(r));return n==="exact"?e===o.join(""):n==="atMost"?[...e].every(r=>o.includes(r)):o.every(r=>e.includes(r))}function Be(e,t){let n=De[t]??De[Ie];return[...e].sort(n)}var I=e=>(t,n)=>{let o=e(t),r=e(n);return o===void 0&&r===void 0?k(t,n):o===void 0?1:r===void 0?-1:r-o||k(t,n)},_t=e=>(t,n)=>{let o=e(t),r=e(n);return o===void 0&&r===void 0?k(t,n):o===void 0?1:r===void 0?-1:o-r||k(t,n)};function k(e,t){return(e.popularity?.edhrecRank??1/0)-(t.popularity?.edhrecRank??1/0)}var De={edge:I(e=>e.edgeScore),momentum:I(e=>e.momentum),worksScore:I(e=>e.worksScore),price:_t(e=>e.price),released:(e,t)=>String(t.releasedAt??"").localeCompare(String(e.releasedAt??""))||k(e,t),name:(e,t)=>e.name.localeCompare(t.name),rank:k,deckCount:I(e=>e.popularity?.deckCount)};var Pe=60;function v(e=a.tab){return e==="discover"?Me:Pe}var jt=140,Ue="mtg-edge-lord:filters",Ft=[{id:"W",label:"W"},{id:"U",label:"U"},{id:"B",label:"B"},{id:"R",label:"R"},{id:"G",label:"G"}],Bt=[{id:"",label:"Any tier"},{id:"edge",label:"Edge (1,000\u20133,000)"},{id:"rare",label:"Rare (500\u20131,000)"},{id:"meta",label:"Meta (top 500)"},{id:"uncharted",label:"Uncharted (3,000+)"}],a={tab:"search",filters:{...ae,...cn()},shown:Pe,expanded:null,detail:null,index:null,data:null,loading:!0,error:null},ze=null,le=document.createElement("div");le.id="mtg-edge-lord-root";var d=le.attachShadow({mode:"open"}),p=document.createElement("div");p.id="mtg-edge-lord-button";var ce=p.attachShadow({mode:"open"});document.body.append(le);zt();Ge();qt();de({checkRemote:!1});function zt(){ce.innerHTML=`<style>${R}</style>
    ...
    <section id="panel" aria-label="MTG Edge Lord" hidden>
      ...
      <main></main>
    </section>`,ce.getElementById("toggle").addEventListener("click",Pt),d.getElementById("status").addEventListener("click",async()=>{a.checking||(a.checking=!0,d.getElementById("status").textContent=We(),await de({force:!0}))}),d.querySelector("nav").addEventListener("click",e=>{let t=e.target.closest("button[data-tab]");t&&(a.tab=t.dataset.tab,a.shown=v(),ue())})}function Ht(){for(let e of document.querySelectorAll('header nav input[aria-label="Search"], header nav input.rbt-input-main')){let t=e.closest('[class*="Navbar_search"], .input-group');if(t?.getBoundingClientRect().width)return t}return null}function Ge(){let e=Ht();if(e){p.classList.remove("floating"),p.previousElementSibling!==e&&e.after(p);return}p.classList.add("floating"),p.parentElement!==document.body&&document.body.append(p)}function qt(){let e=null,t=()=>{clearTimeout(e),e=setTimeout(()=>{Ge(),Z(a.index?.commanders),d.getElementById("panel").hidden||Ve()},100)};new MutationObserver(t).observe(document.body,{childList:!0,subtree:!0}),addEventListener("resize",t)}function Pt(){let e=d.getElementById("panel"),t=e.hidden;t&&Ve(),e.hidden=!t;let n=ce.getElementById("toggle");n.setAttribute("aria-expanded",String(t)),n.classList.toggle("on",t),t&&de()}var He=14;function Ve(){let e=d.getElementById("panel"),t=p.getBoundingClientRect(),n=!p.classList.contains("floating");e.style.setProperty("--mel-top",`${n?Math.round(t.bottom)+8:He}px`),e.style.setProperty("--mel-right",`${He}px`)}async function de({checkRemote:e=!0,force:t=!1}={}){try{a.data=await Ce({checkRemote:e,force:t}),a.index=je(a.data.commanders.commanders),a.error=null}catch(n){a.error=n.message}finally{a.loading=!1,a.checking=!1,ue(),Z(a.index?.commanders)}}function ue(){d.querySelectorAll("nav button").forEach(t=>t.classList.toggle("active",t.dataset.tab===a.tab)),d.getElementById("status").textContent=We();let e=d.querySelector("main");if(a.loading)return void(e.innerHTML='<div class="empty">Loading\u2026</div>');if(a.error)return void(e.innerHTML=`<div class="empty">${s(a.error)}</div>`);e.innerHTML=`
    ...
    <div class="stack" id="results"></div>`,Gt(e),w()}function We(){if(a.checking)return"Checking\u2026";if(!a.data)return"Discovery data unavailable";let e=a.index?.commanders.length??0,t=a.data.stale?"offline":Ut(a.data.checkedAt);return`${e.toLocaleString()} commanders \xB7 ${a.data.manifest.dataVersion}${t?` \xB7 ${t}`:""}`}function Ut(e){let t=Date.now()-Date.parse(e);if(!Number.isFinite(t)||t<0)return"";let n=Math.floor(t/36e5);return n<1?"checked just now":n<24?`checked ${n}h ago`:"checking\u2026"}function Gt(e){e.querySelectorAll("[data-field]").forEach(t=>{let n=t.tagName==="SELECT"||t.type==="checkbox"?"change":"input";t.addEventListener(n,()=>{let o=t.type==="checkbox"?t.checked:t.value;a.filters[t.dataset.field]=o,a.shown=v(),se(),n==="input"?Vt():w()})}),e.querySelectorAll("[data-color]").forEach(t=>t.addEventListener("click",()=>{let n=t.dataset.color,o=a.filters.colors.includes(n)?a.filters.colors.filter(r=>r!==n):[...a.filters.colors,n];a.filters.colors=o,t.classList.toggle("on",o.includes(n)),a.shown=v(),se(),w()})),e.addEventListener("click",t=>{if(t.target.closest("a, button, input, select, label"))return;let n=t.target.closest("[data-open]");n&&open(n.dataset.open,"_blank","noopener")}),e.querySelector("#reset")?.addEventListener("click",()=>{a.filters={...ae},a.shown=v(),se(),ue()})}function Vt(){clearTimeout(ze),ze=setTimeout(w,jt)}function w(){let e=d.getElementById("results"),t=d.getElementById("result-summary");if(!e)return;if(a.tab==="discover"){let r=Oe(Le(a.data.findings.findings).filter(l=>ie(`${l.title} ${l.summary}`,a.filters.query))),i=r.slice(0,a.shown);return t.textContent=r.length>i.length?`Newest ${i.length} of ${r.length.toLocaleString()} finds`:r.length?`${r.length} find${r.length===1?"":"s"}, newest first`:"No finds",e.innerHTML=i.map(on).join("")||B("No finds match that search yet."),qe(e,r.length),j(e)}if(a.tab==="archive"){let r=(a.data.archive?.archive??[]).filter(i=>ie(i.name,a.filters.query));return t.textContent=r.length?`${r.length} commander${r.length===1?"":"s"} surfaced to date`:"Nothing surfaced yet",e.innerHTML=r.slice(0,a.shown).map(an).join("")||B("No commander has been surfaced under that name yet."),j(e)}if(a.tab==="card"){let r=a.data.cards.cards.filter(i=>ie(i.name,a.filters.query));return t.textContent=`${r.length} card${r.length===1?"":"s"}`,e.innerHTML=r.slice(0,a.shown).map(sn).join("")||B("No cards match that search yet."),j(e)}let n=Be(Fe(a.index,a.filters),a.filters.sort),o=n.filter(r=>r.edgeScore!==void 0).length;t.innerHTML=`<strong>${n.length.toLocaleString()}</strong> commanders \xB7 ${o.toLocaleString()} scored \xB7 sorted by ${s(ln(a.filters.sort))}`,e.innerHTML=n.slice(0,a.shown).map(Yt).join("")||B("Nothing matches those filters. Widen the rank band or clear a colour."),qe(e,n.length),j(e)}function qe(e,t){let n=t-a.shown;n<=0||e.insertAdjacentHTML("beforeend",`<button class="more" id="more" type="button">Show ${Math.min(v(),n)} more of ${n.toLocaleString()}</button>`)}function j(e){e.querySelector("#more")?.addEventListener("click",()=>{a.shown+=v(),w()}),e.querySelectorAll("[data-expand]").forEach(t=>t.addEventListener("click",async()=>{let n=t.dataset.expand;a.expanded=a.expanded===n?null:n,a.expanded&&!a.detail&&(a.detail=await Ee(a.data.manifest.dataVersion)),w()}))}function Wt(){let e=a.filters;return`<div class="controls">
    ...
    <div class="row colors">
      ...
      ${Ft.map(t=>`<button type="button" class="pip ${e.colors.includes(t.id)?"on":""}" data-color="${t.id}" aria-label="${t.label}">${t.label}</button>`).join("")}
      <select data-field="colorMode" aria-label="Colour matching">${b(_e.map(t=>[t.id,t.label]),e.colorMode)}</select>
    </div>
    <div class="row">
      ...
      <select data-field="tier" aria-label="Tier">${b(Bt.map(t=>[t.id,t.label]),e.tier)}</select>
    </div>
    <div class="row">
    ...
    <div class="row">
      ...
      <select data-field="sort" aria-label="Sort">${b(_.map(t=>[t.id,t.label]),e.sort)}</select>
      <button type="button" id="reset" class="reset">Reset</button>
    </div>
  </div>`}function Kt(){return`<div class="controls"><input class="query" data-field="query" value="${c(a.filters.query)}" placeholder="Search" aria-label="Search"></div>`}function Yt(e){let t=a.expanded===e.slug,n=z(e.slug),o=(e.themes??[]).slice(0,3);return`<article class="card commander" data-open="${c(n)}">
    <div class="head">
    ...
    ${t?Xt(e):""}
  </article>`}function z(e){return`https://edhrec.com/commanders/${encodeURIComponent(e)}`}function Jt(e){let t=M(e.manaCost);return t||(e.colorIdentity===void 0?"":`<span class="chip">${s(e.colorIdentity||"Colourless")}</span>`)}function Zt(e){let t=e.popularity?.edhrecRank,n=e.popularity?.deckCount;return[t?`#${t.toLocaleString()}`:null,n?`${n.toLocaleString()} decks`:null,e.price!==void 0?`$${e.price.toFixed(2)}`:null].filter(Boolean).join(" \xB7 ")}function Xt(e){return`<div class="detail">
    ${en(e)}
    ${tn(e)}
    ${nn(e)}
  </div>`}function Qt(e){return e.edgeScore!==void 0?`<span class="score">${e.edgeScore}</span>`:e.cohortScore!==void 0?`<span class="score cohort" title="Scored against its set cohort, not the whole format">${e.cohortScore}<small>new</small></span>`:'<span class="score none" title="Not enough evidence to score">\u2014</span>'}function en(e){return e.cohortScore!==void 0?`<p class="cohort-note">New arrival \u2014 scored against its ${s(String(e.cohort?.setCode??"").toUpperCase())} set cohort, not the whole format.</p>
      <ul class="why">${T(e).map(t=>`<li>${s(t)}</li>`).join("")}</ul>`:e.edgeScore===void 0?`<p class="insufficient">Insufficient data \u2014 ${s(E(e))}</p>`:`<ul class="why">${C(e).map(t=>`<li>${s(t)}</li>`).join("")}</ul>`}function tn(e){let t=e.quality;if(!t)return"";let n=[["Bracket fit",t.bracketFit],["Archetype depth",t.archetypeDepth],["Retention",t.retention]].filter(([,o])=>o!==void 0);return n.length?`<div class="chips">${n.map(([o,r])=>`<span class="chip quality">${o} ${Math.round(r*100)}%</span>`).join("")}${e.partialScore?'<span class="chip muted">partial</span>':""}</div>`:""}function nn(e){let t=a.detail?.[e.slug];return t?`<div class="more-detail">
  ...
  </div>`:""}function rn(e){let t=e.cedhListing;if(!t?.sourceUrl)return"";let n={brew:"In the Brewer's Corner",competitive:"Listed as an established competitive deck",outdated:"Listed, but its entry is marked outdated"}[t.section];if(!n)return"";let o=t.updatedAt?` \xB7 entry updated ${s(t.updatedAt.slice(0,10))}`:"";return`<p class="muted"><a href="${c(t.sourceUrl)}" target="_blank" rel="noopener noreferrer">${n} on the cEDH Decklist Database \u2197</a>${o} \u2014 shown, never scored.</p>`}function on(e){let t=[...e.commanders,...e.cards].map(n=>`<span class="chip">${s(n.name)}</span>`).join("");return`<a class="card digest" href="${c(e.source.url)}" target="_blank" rel="noopener noreferrer">
    <div class="credit">
  ...
  </article>`}function sn(e){let t=e.commanders.map(n=>`<a class="chip" href="${c(z(n.slug))}" target="_blank" rel="noopener noreferrer">${s(n.name)} \xB7 ${n.relationshipScore}</a>`).join("");return`<article class="card"><h3>${s(e.name)}</h3><div class="chips">${t}</div>${Ke(e.findingIds)}</article>`}function Ke(e=[]){let t=new Set(e),n=(a.data.resources.resources??[]).filter(o=>t.has(o.findingId)&&typeof o.url=="string"&&o.url.startsWith("https://")).slice(0,3);return n.length?`<p class="sources">${n.map(o=>`<a href="${c(o.url)}" target="_blank" rel="noopener noreferrer">${s(o.creator)} \xB7 ${dn(o.resourceDepth)} \u2197</a>`).join(" \xB7 ")}</p>`:""}function ie(e,t){let n=String(t??"").trim().toLowerCase();return!n||String(e).toLowerCase().includes(n)}function se(){try{sessionStorage.setItem(Ue,JSON.stringify(a.filters))}catch{}}function cn(){try{return JSON.parse(sessionStorage.getItem(Ue))??{}}catch{return{}}}function ln(e){return _.find(t=>t.id===e)?.label??_[0].label}function H(e){return String(e).replaceAll("-"," ").replace(/\b[a-z]/g,t=>t.toUpperCase())}function b(e,t){return e.map(([n,o])=>`<option value="${c(n)}" ${String(t)===String(n)?"selected":""}>${s(o)}</option>`).join("")}function F(e,t){return`<button type="button" data-tab="${e}" class="${a.tab===e?"active":""}">${t}</button>`}function dn(e){return s(String(e).replaceAll("_"," "))}function B(e){return`<div class="empty">${s(e)}</div>`}function s(e){let t=document.createElement("span");return t.textContent=String(e),t.innerHTML}function c(e){return s(e).replaceAll('"',"&quot;")}})();
